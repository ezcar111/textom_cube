...
class DaumWeb(DaumBase):
    # url_search        = "http://search.daum.net/search?w=web&enc=utf8&q=%(query)s&m=board&%(date_query)s&p=%(start)d&DA=PGD"
    url_search        = "https://search.daum.net/search?w=fusion&col=web&q=%(query)s&DA=STC&p=%(start)d&period=u&%(date_query)s&sort=recency"
    def search(self, 
        query, 
        idx_num, 
        stop, 
        ...
        # out_file = open(file_name+"file_daum_data/"+uniq_file_name+"_daum_web.txt","a")
