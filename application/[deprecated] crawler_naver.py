...
def factory(task_no, row, data_type_name):
    ...
    data_type, naver_poly = get_data_poly(data_type_name) 
    if naver_poly is not None:
        exec(f"UPDATE scraw_naver_datainfo SET {data_type}_starttime = now() where idx={task_no}")

        out_filepath = get_out_filepath(data_type)
        create_file_name, item_count = naver_poly.search(
            keyword, 
            idx_num = str(task_no), 
            stop=1000, 
        ...
        exec(f"UPDATE scraw_naver_datainfo SET {data_type}_endtime = now() where idx={task_no}")
...
def get_out_filepath(data_type):
    ...
    file_path =f"{scrap_data_save_path}/{uniq_file_name}_naver_{data_type}.txt"
    return file_path


def save(task_no, data_type, create_file_name, item_count):
    result = exec(f"SELECT keyword_list_idx from scraw_naver_datainfo where idx = {task_no}")
    ...
    if row is not None:
        ...
        out_file.close()
        exec(f"UPDATE scraw_naver_datainfo SET {data_type}_countnum_craw={item_count} WHERE idx = {task_no}")
        ...
        try :
            ...
            if row_user is not None:
                ...
                query = "INSERT INTO scrw_use_size (" \
                        ...
                        ") VALUES (" \
                        f"{k_idx}, '{re.escape(keyword)}', '{user_id}', 'naver', '{data_type}', {file_size}, '{date_start}', '{date_end}', 'channel'" \
                        ")"
                exec(query)
