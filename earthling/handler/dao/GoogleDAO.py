...
class GoogleDAO(BaseDAO):
    ...
    def select_wait_task(self):
        ...
        query = "SELECT X.* " \
                ...
                "    web_data, news_data, facebook_data " \
                "  FROM scraw_google_datainfo " \
                "  WHERE " \
                f"   {test_index_query} (web_data  = 'Y' OR news_data = 'Y' OR facebook_data = 'Y') " \
                "  ORDER BY idx ASC " \
                ") AS X " \
    ...
    def update_state_to_finish(self, no, data_type_name):
        try:
            ...
            result = exec(f"SELECT keyword_list_idx, web_data, news_data, facebook_data FROM scraw_google_datainfo WHERE idx={no}")
            if len(result) > 0:
                k_idx = result[0]["keyword_list_idx"]
                crawl_state = [
                    result[0]["web_data"],
                    result[0]["news_data"],
                    result[0]["facebook_data"]
                ]
