...
class DaumDAO(BaseDAO):
    ...
    def select_wait_task(self):
        ...
        query = "SELECT X.* " \
                ...
                "    (SELECT daum_search_ip FROM keyword_list WHERE idx = keyword_list_idx) AS k_ip," \
                "    web_data, news_data, blog_data, cafe_data " \
                "  FROM scraw_daum_datainfo " \
                "  WHERE " \
                f"   {test_index_query} (web_data  = 'Y' OR news_data = 'Y' OR blog_data = 'Y' OR cafe_data = 'Y') " \
                "  ORDER BY idx ASC " \
                ") AS X " \
    ...
    def update_state_to_finish(self, no, data_type_name):
        try:
            ...
            result = exec(f"SELECT keyword_list_idx, web_data, news_data, blog_data, cafe_data FROM scraw_daum_datainfo WHERE idx={no}")
            if len(result) > 0:
                ...
                if ('Y' not in crawl_state) and ('S' not in crawl_state):
                    query = f"UPDATE keyword_list SET daum_status = 'merge' WHERE idx = {k_idx}"
                    exec(query)
        except:
            pass